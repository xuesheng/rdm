...
class RequirementFileController extends Controller
{
    ...
    public function upload(Request $request)
    {
        ...
        if (!$request->hasFile('code_file')) {
            return redirect("/requirement/edit/{$id}")->with('msg', '必须选择上传文件！');
        }

        if (!$request->file('code_file')->isValid()){
            return redirect("/requirement/edit/{$id}")->with('msg', '文件上传出现异常！');
        }
        ...
        if ($path === false) {
            return redirect("/requirement/edit/{$id}")->with('msg', '文件上传失败！');
        }
        ...
        if ($requirementsFiles = RequirementsFiles::where('name', $name)->where('requirement_id', $id)->first()) {
            ...
            if ($requirementsFiles->path == $path) {
                return redirect("/requirement/edit/{$id}")->with('msg', '文件未发生变化，无须重复上传！');
            }
            ...
            if (!$requirementsFiles->save()) {
                return redirect("/requirement/edit/{$id}")->with('msg', '文件更新失败！');
            }

            return redirect("/requirement/edit/{$id}")->with('msg', '文件更新成功！');
        }
        ...
        return redirect("/requirement/edit/{$id}")->with('success_msg', '文件上传成功！');
